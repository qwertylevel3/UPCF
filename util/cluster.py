...
def run():
    ...
    y_two_means = MiniBatchKMeans(n_clusters=3).fit_predict(X)
    ...
    y_dbscan = DBSCAN(eps=.2).fit_predict(X)
    y_affinity_propagation = AffinityPropagation(damping=.9,
                                                       preference=-100).fit_predict(X)
    ...
    plt.subplot(334)
    plt.scatter(X[:, 0], X[:, 1], c=y_two_means)
    plt.subplot(335)
